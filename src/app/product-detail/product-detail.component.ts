...
onsignup() {
  ...
  const params = new HttpParams()
    ...
    .set('privatetransferprice', this.productDetails?.privatetransferprice || 0);

  // Make API call
  this.http.get<any>('http://localhost:4000/api/booknow', { 
    params,
    ...
    next: (res) => {
      ...
      if (res.status === 409) {
        if (res.value !== undefined) {
          ...
          alert(`❌ Sorry! Only ${res.value} spots available for selected date.`);
        } else if (res.message) {
    ...
    error: (err) => {
      ...
      if (err.status === 409) {
        if (err.error?.value !== undefined) {
          alert(`❌ Only ${err.error.value} spots available for selected date.`);
  ...
  onPrivateBookNow() {
    ...
    console.log('Sending private booking request with params:', params.toString());
    ...
    this.http.get<any>('http://localhost:4000/api/booknow', { 
      ...
      next: (res) => {
        if (res.status === 409) {
          const errorMsg = res.message || 'Private transfer not available for the selected date';
          alert(`❌ ${errorMsg}`);
          return;
        }
        if (res.status === 200 && res.cart) {
