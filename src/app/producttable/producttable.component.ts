...
export class ProducttableComponent implements OnInit {
  ...
  loadCartFromAPI(): void {
    this.http.get<any>('http://localhost:4000/api/cart', { withCredentials: true })
      .subscribe({
        next: (res) => {
  ...
  onSubmit(): void {
    ...
    if (this.cartItems.length > 0) {
      ...
      productsArray = this.cartItems.map(item => ({
        ...
        privateAdult: item.transfertype === 'Private' ? item.privateAdult : null,
        privateChild: item.transfertype === 'Private' ? item.privateChild : null,
        transfertype: item.transfertype,
        transferPrice: item.transfertype === 'Shared' ? item.transferPrice : null,
        privatetransferprice: item.transfertype === 'Private' ? item.privatetransferprice : null,
      ...
      productsArray = [{
        ...
        transferPrice: form.Transfer === 'Shared' ? this.productData.sharedTransferPrice : null,
        privatetransferprice: form.Transfer === 'Private' ? this.productData.privatetransferprice : null,
        total: form.Total,
        order_date: new Date().toLocaleDateString()
      }];
    } else {
...
deleteConfirmed(): void {
  ...
  this.http
    .get(`http://localhost:4000/remove-from-cart?productId=${item.productId}`, {
      withCredentials: true,
    })
    .subscribe({
      next: (res: any) => {
        if (res.status === 200 && res.message === 'if the cart is now empty') {
